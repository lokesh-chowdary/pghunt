...
export default function PGCardDetails() {
  ...
  // SEO-related data (using existing pgName from line 191)
  const pgDescription = `${pgName} - ${pg?.category || 'PG'} accommodation in ${pg?.city || ''}${pg?.area ? `, ${pg?.area}` : ''}. ${pg?.address || ''}. Starting from ₹${pg?.price || 'Contact for price'}. ${pg?.amenities?.length ? `Amenities: ${pg.amenities.join(', ')}.` : ''}`;
  const pgImage = pg?.images && Array.isArray(pg.images) && pg.images.length > 0 
    ? getAllImageUrls(pg.images)[0] 
    : '/og-image.jpg';
  const pgUrl = typeof window !== 'undefined' ? window.location.href : '';

  const structuredData = pg ? {
    ...
    address: {
      ...
      addressRegion: pg.city || '',
      addressCountry: 'IN'
    },
    priceRange: pg.price ? `₹${pg.price}` : 'Contact for price',
    amenityFeature: pg.amenities?.map((amenity: string) => ({
  ...
  return (
    <>
      <SEO
        title={`${pgName} - PG Details | PG Finder`}
        description={pgDescription}
        keywords={`${pgName}, PG in ${pg?.city || ''}, ${pg?.category || 'PG'} accommodation, PG near ${pg?.area || ''}, verified PG`}
        image={pgImage}
        url={pgUrl}
        type="article"
      ...
      <div className="min-h-screen bg-[#F5F7FB]">
        ...
        <div className="grid md:grid-cols-3 gap-6 md:gap-6 lg:gap-8">
          {/* LEFT COLUMN */}
          <div className="md:col-span-2 space-y-6 md:space-y-6 lg:space-y-7">
              ...
              <div className="relative">
                <div className="h-60 sm:h-72 md:h-80 lg:h-80">
                  {activeImageUrl ? (
                    <img
                      src={activeImageUrl}
                      alt={`${pgName} - ${pg?.category || 'PG'} accommodation in ${pg?.city || ''}${pg?.area ? `, ${pg?.area}` : ''} - Image ${activeImage + 1}`}
